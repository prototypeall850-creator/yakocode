...
async fn main() -> Result<()> {
    ...
    if cli.model.is_none() {
        if let Some(p) = cli.provider.as_deref() {
            if p.eq_ignore_ascii_case("meta") {
                cli.model = Some("meta:muse-spark-1.3".into());
            }
        }
    }
    ...
    let working_mode = if cli.serve.is_some() {
        WorkingMode::Serve
    } else if cli.tui || (text.is_none() && cli.file.is_empty()) {
        WorkingMode::Repl
    } else {
        WorkingMode::Cmd
    };
...
async fn run(config: GlobalConfig, cli: Cli, text: Option<String>) -> Result<()> {
    ...
    match is_repl {
        ...
        true => {
            ...
            if cli.tui {
                start_tui(&config, cli.allow_dir.as_deref()).await
            } else {
...
/// yakocode: Ratatui TUI di atas full backend (roles/sessions/agents/RAG/functions),
/// plus MCP filesystem untuk /read dan /ls.
async fn start_tui(config: &GlobalConfig, allow_dir: Option<&str>) -> Result<()> {
    use crossterm::{execute, terminal::*};
    use ratatui::{backend::CrosstermBackend, Terminal};

    let dir = allow_dir.unwrap_or(".").to_string();
    let base = std::path::PathBuf::from(&dir)
        .canonicalize()
        .unwrap_or(std::path::PathBuf::from(&dir));
    ...
    let res = app::run_tui(&mut term, config.clone(), base).await;
