...
pub struct Cli {
    ...
    /// Launch the Ratatui TUI instead of the reedline REPL (yakocode)
    #[clap(long)]
    pub tui: bool,
    /// Folder allowed for the MCP filesystem server (yakocode, used with --tui)
    #[clap(long, value_name = "DIR")]
    pub allow_dir: Option<String>,
    /// Shorthand provider selector: openai | meta (yakocode).
    /// `meta` maps to model `meta:muse-spark-1.3` unless --model is given.
